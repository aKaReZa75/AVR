...
int main(void)
{
  ...
  while(1)
  {
    Segment[0] = (Counter/1000);       // Hezargan (Digit1)
    Segment[1] = ((Counter%1000)/100); // Sadgan (Digit2)
    Segment[2] = ((Counter%100)/10);   // Dahgan (Digit3)
    Segment[3] = (Counter%10);         // Yekan (Digit4)

    for(fourLoopCounter = 0; fourLoopCounter <= 3; fourLoopCounter++)
    {
      bitChange(Seg7_A_Control,  Seg7_A_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 0));
      bitChange(Seg7_B_Control,  Seg7_B_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 1));
      bitChange(Seg7_C_Control,  Seg7_C_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 2));
      bitChange(Seg7_D_Control,  Seg7_D_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 3));
      bitChange(Seg7_E_Control,  Seg7_E_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 4));
      bitChange(Seg7_F_Control,  Seg7_F_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 5));
      bitChange(Seg7_G_Control,  Seg7_G_Pin,  bitCheck(Segments_CC[Segment[fourLoopCounter]], 6));
      switch (fourLoopCounter)
      {
        case 0:
